## Hi, I'm Holland 👋

Software Engineer building thoughtful software that solves real-world problems through automation and full-stack development.

I enjoy turning tedious, repetitive workflows into reliable software while continuously learning new technologies and engineering practices.

📍 Denver, CO
🎓 B.S. Software Engineering | M.S. Software Engineering (DevOps)
💼 Open to Software Engineering opportunities

### 🔭 I’m currently working on 
- 📚 **Corelato** *(private)* — A full-stack learning platform for notes, flashcards, courses, and long-term knowledge management.
- 🤖 **Job Radar** — An automation platform that monitors multiple ATS job boards and delivers personalized job alerts.
- 📬 **Compartment Mail** — A local-first email client focused on privacy and workflow isolation.

### 🌱 I’m currently learning 

- DevOps through my M.S. in Software Engineering
- Cloud infrastructure and deployment
- Building reliable, maintainable software systems

### ⭐ Featured Public Projects

- 🚗 Colorado DMV Appointment Watcher
- 🤖 Job Radar
- 🧪 QA Wolf Take-Home Assessment
- 📂 Compartment Mail

### 💻 Tech

TypeScript • React • Next.js • Python • FastAPI • Java • PostgreSQL • Playwright • Docker

### 🚀 Recent Highlights

- 🎓 Graduated with a B.S. in Software Engineering (2026)
- ☁️ AWS Certified Cloud Practitioner
- 📚 Currently pursuing an M.S. in Software Engineering — DevOps
- 🧪 Built several production-ready full-stack applications and automation tools

### 📫 Connect

🌐 Portfolio

💼 LinkedIn

📧 holland@hollandmwesley.com

<!--
**hmw55/hmw55** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
